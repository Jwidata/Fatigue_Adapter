...
def main():
    ...
    model = LSTMGazeModel(input_size=input_size, hidden_size=64, num_layers=2, dropout=0.1).to(device)
    ...
    weights_path = Path("configs/lstm_gaze_weights.pt")
    torch.save(model.state_dict(), weights_path)
    history_path = Path("artifacts/lstm_history.json")
    history_path.parent.mkdir(parents=True, exist_ok=True)
    history_path.write_text(json.dumps(history, indent=2), encoding="utf-8")
    config_path = Path("artifacts/lstm_config.json")
